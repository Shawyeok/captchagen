...
module.exports = function(canvas, opt) {
  ...
  opt.text.split('').forEach(function(letter, idx) {
    ...
    if (typeof opt.fontsize === 'number') {
      ...
      console.warn("Invalid fontsize: %s, use default value %s", JSON.stringify(opt.fontsize), JSON.stringify(constants.fontsize));
      min = constants.fontsize[0];
      max = constants.fontsize[1];
    }
    // set font
